...
class Nginx < Formula
  ...
  def install
    ...
    guest_prefix = "/home/linuxbrew/.linuxbrew/opt/nginx"
    ...
    kandelo_wasm_build do |root|
      ...
      configure_runner = Pathname(__dir__).parent/"Kandelo/formula_support/run-network-wasm.ts"
      ...
      system "./configure",
        ...
        "--prefix=#{guest_prefix}",
        "--sbin-path=#{guest_prefix}/bin/nginx",
        "--conf-path=#{guest_prefix}/conf/nginx.conf",
      ...
      instrumented = buildpath/"objs/nginx.instrumented"
      ...
      system "bash", "-c", <<~SH
        ...
        if [ "$artifact_abi" != "$expected_abi" ]; then
          echo "ERROR: nginx ABI $artifact_abi does not match Kandelo ABI $expected_abi" >&2
          exit 1
        fi
        ...
        if ! wasm_has_complete_fork_instrumentation #{instrumented.to_s.shellescape}; then
          echo "ERROR: nginx has incomplete fork instrumentation" >&2
          exit 1
        fi
      SH

      bin.install instrumented => "nginx"
  ...
  test do
    ...
    assert_match "nginx version: nginx/1.24.0", version_output
    ...
    (testpath/"nginx.conf").write <<~EOS
      ...
      http {
        include /home/linuxbrew/.linuxbrew/opt/nginx/conf/mime.types;
        access_log off;
        gzip on;
        gzip_min_length 1;
    ...
    responses = kandelo_run_http_service(
      ...
      mounts:   {
        ...
        "/home/linuxbrew/.linuxbrew/opt/nginx" => opt_prefix.to_s,
      },
