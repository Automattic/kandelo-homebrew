...
class Libxml2 < Formula
  ...
  def install
    ...
    # CMake's Generic platform does not infer that Kandelo musl provides iconv
    # in libc. Preserve that target fact in libxml2's installed CMake config.
  ...
  test do
    ...
    source.write <<~C
      ...
      static void *parse_document(void *argument) {
        ...
        if (content == NULL || xmlStrcmp(content, BAD_CAST "caf\\xc3\\xa9") != 0) {
          *result = 3;
          return NULL;
        }
      ...
      int main(void) {
        static const char xml[] =
          "<?xml version='1.0' encoding='ISO-8859-1'?><root>caf\\xe9</root>";
        gzFile compressed;
        pthread_t thread;
        int result = -1;
